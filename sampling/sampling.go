...
type Sampling interface {
	Start(ctx context.Context, initialPeers []Peer)

	SelectPeer() Peer
	ReceiveView(view View, from Peer) []byte
	fillView(peers ...Peer) // view to select peers from based on strategy
	passive()
	active()
	GetView() *View
...
type peerSampling struct {
	...
	peers          map[string]Peer // udp-address:peer
	ctx            context.Context
}
...
func (p *peerSampling) Start(ctx context.Context, initialPeers []Peer) {
	p.ctx = ctx
...
func (p *peerSampling) ReceiveView(view View, from Peer) []byte {
...
func (p *peerSampling) fillView(peers ...Peer) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *peerSampling) SelectPeer() Peer {
	address := ""
...
func Init(self string, identifier string, loggerOn bool, strategy PeerSamplingStrategy) Sampling {
	...
	ps.lock.Lock()
	ps.peers[ps.selfIdentifier] = Peer{
		UdpAddress:        self,
